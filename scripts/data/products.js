...
class Clothing extends Product {
    ...
    extraInfo() {
        return `
       <a href="${this.sizeChartLink}" target="_blank">Clothing Size</a>
    `;
    }
}
...
class Appliance extends Product {
    constructor(productItem) {
        super(productItem);
        this.instructionLink = productItem.instructionLink;
        this.warrantyLink = productItem.warrantyLink;
    }
    extraInfo() {
        return `<a href='${this.instructionLink}' >Instructions</a> <a href='${this.warrantyLink}' >warranty</a>`;
    }
}
export function loadProductsFetch() {
    const promise = fetch('https://supersimplebackend.dev/products').then((response) => {
        ...
        products = productsData.map((productItems) => {
            if (productItems.type === 'clothing') {
                return new Clothing(productItems);
            }
            else if (productItems.type === 'appliance') {
                return new Appliance(productItems);
            }
...
export function loadProducts(fun) {
    ...
    xhr.addEventListener('load', () => {
        ...
        products = response.map((productItems) => {
            if (productItems.type === 'clothing') {
                return new Clothing(productItems);
            }
            else if (productItems.type === 'appliance') {
...
export const products = [
  ...
  {
    id: "id2",
    image: "images/products/beyblade.webp",
    name: "beyblade",
    rating: {
      stars: 4.0,
      count: 2
